...
set(WITH_BUILD CACHE STRING "Build OS triplet")
...
if((CORE_PLATFORM_NAME_LC IN_LIST VALID_LINUX_PLATFORMS_LIST OR CORE_PLATFORM_NAME_LC STREQUAL rbpi) AND NOT WITH_TOOLCHAIN)
  message(FATAL_ERROR "No toolchain specified. For example -DWITH_TOOLCHAIN=/usr/arm-linux-gnueabi")
endif()

if((CORE_PLATFORM_NAME_LC IN_LIST VALID_LINUX_PLATFORMS_LIST OR CORE_PLATFORM_NAME_LC STREQUAL rbpi) AND NOT IS_ABSOLUTE ${WITH_TOOLCHAIN})
...
if(NOT WITH_BUILD)
  message(FATAL_ERROR "No build triplet specified. For example -DWITH_BUILD=x86_64-redhat-linux")
endif()

# rough check
if(NOT WITH_BUILD MATCHES "[^ ]*-[^ ]*-[^ ]*")
  message(FATAL_ERROR "Specified build triplet not valid. For example -DWITH_BUILD=x86_64-redhat-linux")
endif()

if(NOT WITH_HOST)
  message(FATAL_ERROR "No host triplet specified. For example -DWITH_HOST=arm-linux-gnueabi")
endif()

# rough check
if(NOT WITH_HOST MATCHES "[^ ]*-[^ ]*-[^ ]*")
  message(FATAL_ERROR "Specified host triplet not valid. For example -DWITH_HOST=arm-linux-gnueabi")
endif()
...
if(CORE_PLATFORM_NAME_LC STREQUAL android)
  ...
  if(NOT IS_ABSOLUTE ${WITH_SDK_PATH})
    message(FATAL_ERROR "WITH_NDK_PATH must be an absolute path. For example -DWITH_NDK_PATH=/opt/android-tools/android-sdk-linux")
  endif()

  if(NOT EXISTS ${WITH_SDK_PATH}/tools/bin/sdkmanager)
    message(FATAL_ERROR "WITH_NDK_PATH is invalid. For example -DWITH_NDK_PATH=/opt/android-tools/android-sdk-linux")
  endif()
