...
function(set_environment)
  ...
  if(BUILD_NAME STREQUAL NATIVE)
    set(CMAKE_C_FLAGS "-I${INSTALL_PREFIX_NATIVE}/include")
    set(CMAKE_CXX_FLAGS "-I${INSTALL_PREFIX_NATIVE}/include")
    set(CMAKE_EXE_LINKER_FLAGS "-L${INSTALL_PREFIX_NATIVE}/lib")
  endif()
  ...
  set(BUILD_ENV_TARGET env
                       ...
                       PATH=${INSTALL_PREFIX_NATIVE}/bin:$ENV{PATH}

                       AUTOM4TE=${INSTALL_PREFIX_NATIVE}/bin/autom4te
                       AUTOMAKE=${INSTALL_PREFIX_NATIVE}/bin/automake
                       AUTOCONF=${INSTALL_PREFIX_NATIVE}/bin/autoconf
                       ACLOCAL=${INSTALL_PREFIX_NATIVE}/bin/aclocal
                       ACLOCAL_PATH=${INSTALL_PREFIX_TARGET}/share/aclocal:${INSTALL_PREFIX_NATIVE}/share/aclocal
                       AUTOPOINT=${INSTALL_PREFIX_NATIVE}/bin/autopoint
                       AUTOHEADER=${INSTALL_PREFIX_NATIVE}/bin/autoheader
                       AUTORECONF=${INSTALL_PREFIX_NATIVE}/bin/autoreconf

                       LIBTOOL=${INSTALL_PREFIX_NATIVE}/bin/libtool
                       LIBTOOLIZE=${INSTALL_PREFIX_NATIVE}/bin/libtoolize

                       PKG_CONFIG=${INSTALL_PREFIX_NATIVE}/bin/pkg-config
  ...
  set(AUTOTOOLS_CROSS_ARGS_TARGET --build=${WITH_BUILD} --host=${WITH_HOST} PARENT_SCOPE)
  ...
  set(AUTOTOOLS_CROSS_ARGS_NATIVE PARENT_SCOPE)
  set(CMAKE_TOOLCHAIN_FILE_NATIVE PARENT_SCOPE)
  set(MESON_TOOLCHAIN_FILE_NATIVE PARENT_SCOPE)

  set(MESON_COMMAND_TARGET ${INSTALL_PREFIX_NATIVE}/bin/meson PARENT_SCOPE)
  set(NINJA_COMMAND_TARGET ${INSTALL_PREFIX_NATIVE}/bin/ninja PARENT_SCOPE)

  set(MESON_COMMAND_NATIVE meson PARENT_SCOPE)
  set(NINJA_COMMAND_NATIVE ninja PARENT_SCOPE)

  # PKG_AUTORECONF ON can only be used after autoconf-native is built
  set(AUTORECONF_COMMAND_TARGET ${INSTALL_PREFIX_NATIVE}/bin/autoreconf PARENT_SCOPE)
  set(AUTORECONF_COMMAND_NATIVE autoreconf PARENT_SCOPE)

  set(PATCH_COMMAND_TARGET ${PATCH_COMMAND} PARENT_SCOPE)
  set(PATCH_COMMAND_NATIVE patch PARENT_SCOPE)
...
function(add_depends)
  ...
  if(PKG_TOOLCHAIN STREQUAL "autotools")
    ...
    set(PKG_INSTALL_COMMAND ${BUILD_ENV_${BUILD_NAME}} $(MAKE) install ${PKG_MAKE_INSTALL_OPTS_${BUILD_NAME}} ${LOG_OUTPUT_INSTALL})
  ...
  elseif(PKG_TOOLCHAIN STREQUAL "meson")
    ...
    set(PKG_INSTALL_COMMAND ${BUILD_ENV_${BUILD_NAME}} ${NINJA_COMMAND_${BUILD_NAME}} -v install ${PKG_MAKE_INSTALL_OPTS_${BUILD_NAME}} ${LOG_OUTPUT_INSTALL})
  ...
  if(PKG_TOOLCHAIN MATCHES "autotools|cmake|meson|custom")
    ...
    if(PKG_TOOLCHAIN STREQUAL "meson" AND BUILD_NAME STREQUAL "TARGET")
      ...
      if(NOT TARGET generate-meson-crossfile)
        ...
        # Meson packages cannot generate the crossfile until Python-native is built!
        message(VERBOSE "adding dependency Python-native to target generate-meson-crossfile")
        add_dependencies(generate-meson-crossfile Python-native)
      endif()
  ...
  if(PKG_AUTORECONF)
    ExternalProject_Add_Step(${PKG_NAME}-${BUILD_NAME_LC} autoreconf
                             COMMAND ${AUTORECONF_COMMAND_${BUILD_NAME}} -ivf ${LOG_OUTPUT_AUTORECONF}
    ...
    if(BUILD_NAME STREQUAL TARGET)
      message(VERBOSE "adding dependency autoconf-native-meta to target ${PKG_NAME}-${BUILD_NAME_LC}-autoreconf of ${PKG_NAME}-${BUILD_NAME_LC}")
      ExternalProject_Add_StepDependencies(${PKG_NAME}-${BUILD_NAME_LC} autoreconf autoconf-native-meta)
    endif()
  endif()
  ...
  if(PKG_PATCHES OR PKG_PATCHES_${BUILD_NAME})
    ...
    if(NOT WITH_KODI_DEPENDS)
      set(WITH_KODI_DEPENDS ${CMAKE_SOURCE_DIR})
    endif()
  ...
  foreach(depend ${PKG_DEPENDS_${BUILD_NAME}})
    string(REGEX MATCH "-native|-target" DEPEND_SUFFIX ${depend})
